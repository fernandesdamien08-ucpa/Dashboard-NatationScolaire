...
if not df_raw.empty:
    ...
    with g1:
        st.subheader("🏆 Répartition des Diplômes")
        if "Diplome" in df.columns:
    ...
    with g2:
        st.subheader("📈 Moyenne Globale")
        if len(df.columns) > 30:
            c_deb, c_fin = df.columns[28], df.columns[30]
            m_deb = pd.to_numeric(df[c_deb], errors='coerce').mean()
            m_fin = pd.to_numeric(df[c_fin], errors='coerce').mean()
    ...
    with c_pap:
        st.subheader("📊 Glissement des Notes")
        if len(df.columns) > 30:
            d_cnt = df[c_deb].value_counts().reindex(range(13), fill_value=0)
            f_cnt = df[c_fin].value_counts().reindex(range(13), fill_value=0)
            
            fig = go.Figure()
            fig.add_trace(go.Bar(y=d_cnt.index, x=d_cnt.values*-1, name='Début', orientation='h', marker_color='#adb5bd', 
                                 text=d_cnt.values, textposition='auto', hovertemplate='<b>Début</b><br>Note: %{y}<br>Élèves: %{text}<extra></extra>'))
            fig.add_trace(go.Bar(y=f_cnt.index, x=f_cnt.values, name='Fin', orientation='h', marker_color='#00a896', 
                                 text=f_cnt.values, textposition='auto', hovertemplate='<b>Fin</b><br>Note: %{y}<br>Élèves: %{text}<extra></extra>'))
            
            fig.update_layout(barmode='overlay', bargap=0.1, 
                              xaxis=dict(showticklabels=False, title="Nb Élèves"), 
